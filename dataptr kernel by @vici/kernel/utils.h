...
namespace utils
{
    void* get_system_information(SYSTEM_INFORMATION_CLASS information_class)
    {
        ...
        void* info = ExAllocatePoolZero(NonPagedPool, size, (xorstr('shit')));
        if (!info)
            return nullptr;
 ...
 #define to_lower(text) ((text >= (char*)(xorstr('A')) && text <= (char*)(xorstr('Z'))) ? (text + 32) : text)
